...
| 21 | Telegram API | `sendData()` не работает через меню-кнопку — заявки не отправлялись | tg-app/index.html |
| 22 | TMA UI | Автопрокрутка к слотам при инициализации скрывала календарь от пользователя | tg-app/index.html |
| 23 | TMA UI | CSS `aspect-ratio` + `grid 1fr` даёт overflow — последние колонки уходят за экран | tg-app/index.html |
| 24 | Панель мастера | Записи без username показывали «Клиент» без альтернативного идентификатора | admin/index.html |
| 25 | Панель мастера | Записи сортировались от новых к старым — неудобно для рабочего расписания | api/admin.py |
...
## 21. `tg.sendData()` не работает через меню-кнопку бота

**Что случилось:**
Клиенты, открывавшие TMA через синюю кнопку "Записаться" слева от поля ввода (меню-кнопка), не могли отправить заявку — она молча терялась.

**Причина:**
`tg.sendData()` работает **только** когда TMA открыта через `KeyboardButton` (reply keyboard). Меню-кнопка (`setChatMenuButton`) и прямые ссылки — не поддерживают `sendData()`.

**Решение:**
Заменить `tg.sendData()` на прямой `fetch()` к API:
```js
// ВМЕСТО:
tg.sendData(JSON.stringify(payload));

// ПРАВИЛЬНО:
const resp = await fetch(`${API_BASE}/api/v1/${masterSlug}/bookings`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify(body),
});
```
Бэкенд сам уведомляет мастера и клиента через Telegram Bot API.

**Правило:**
Для любого TMA с несколькими точками входа (меню-кнопка, reply keyboard, ссылка) — использовать прямые API-запросы вместо `sendData()`. `sendData()` слишком ограничен по контексту.

---

## 22. Автопрокрутка при инициализации скрывала UI от пользователя

**Что случилось:**
После замены горизонтальной полосы дат на календарь пользователь открывал экран и видел только слоты времени — календарь был скрыт вверху за пределами экрана.

**Причина:**
При инициализации вызывался `selectDate(firstDate)`, который запускал `scrollIntoView()` к секции слотов. Пользователь никогда не видел сам календарь.

**Решение:**
```js
// При рендере — НЕ прокручивать:
if (firstDate) selectDate(firstDate, false);

// При нажатии пользователем — прокручивать:
function selectDate(dateStr, scroll = true) {
  // ...
...
**Правило:**
Автоматические действия при инициализации (выделение, прокрутка, фокус) не должны скрывать от пользователя основной контент экрана. Прокрутка — только в ответ на явное действие пользователя.

---

## 23. CSS `grid` + `aspect-ratio` даёт overflow на мобильных

**Что случилось:**
Календарь показывал только 5 из 7 колонок — последние 2 (Сб, Вс) уходили за правый край экрана.

**Причина:**
`grid-template-columns: repeat(7, 1fr)` + `aspect-ratio: 1` на ячейках. В некоторых WebView `1fr` вычисляется без учёта `gap`, что приводит к переполнению. `aspect-ratio` добавляет дополнительную ширину.

**Решение:**
```css
/* НЕПРАВИЛЬНО — может давать overflow: */
grid-template-columns: repeat(7, 1fr);
.cal-day { aspect-ratio: 1; }

/* ПРАВИЛЬНО — minmax(0,1fr) предотвращает overflow: */
grid-template-columns: repeat(7, minmax(0, 1fr));
.cal-day { height: 38px; } /* фиксированная высота вместо aspect-ratio */
```

**Правило:**
Для CSS Grid с фиксированным числом колонок использовать `minmax(0, 1fr)` вместо `1fr`. Избегать `aspect-ratio` в ячейках grid — заменять на фиксированную высоту.

---

## 24. Записи без Telegram-данных показывают «Клиент» без деталей

**Что случилось:**
Запись, созданная из браузера (без Telegram-контекста), показывалась в панели мастера как «Клиент» без какой-либо идентификации. Мастер не знала кто записался.

**Причина:**
TMA использует `tg.initDataUnsafe.user` для получения данных клиента. В браузере этот объект недоступен — все поля пустые.

**Решение:**
Каскадный fallback в панели мастера:
```js
const client = b.client_username
  ? `@${b.client_username}`
  : (b.client_name || (b.client_telegram_id ? `tg:${b.client_telegram_id}` : 'Без аккаунта'));
```

**Правило:**
Всегда предусматривать несколько уровней fallback для идентификации пользователя: username → имя → telegram_id → «Без аккаунта». Никогда не полагаться только на один источник данных.

---

## 25. Сортировка записей от новых к старым неудобна для расписания

**Что случилось:**
В панели мастера записи шли от последних к первым (desc). Мастер видела сначала далёкое будущее, а ближайшие записи — внизу.

**Причина:**
`order_by(Booking.booking_date.desc())` — стандартная сортировка для «последние действия», но неудобная для расписания.

**Решение:**
```python
# Для расписания — хронологический порядок:
.order_by(Booking.booking_date.asc(), Booking.booking_time.asc())
```

**Правило:**
Для расписания/календаря — сортировка по возрастанию (ближайшее сверху). Для журнала действий/истории — по убыванию (последнее сверху). Определять тип UI перед выбором сортировки.
